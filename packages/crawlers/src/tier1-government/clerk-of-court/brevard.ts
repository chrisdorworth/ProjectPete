...
export class BrevardClerkCrawler extends BaseClerkCrawler {
  ...
  protected parseSearchResults(html: string, baseUrl: string): CaseRecord[] {
    const rows = extractTable(html, "table.caseResults, table#results, table.search-results");
    ...
    return rows.map((row, index) => ({
      ...
      filingDate: row["file_date"] ?? row["filed_date"] ?? row["col_2"] ?? "",
      parties: this.extractParties(row),
      amount: row["amount"] ?? row["judgment"] ?? null,
      description: row["description"] ?? row["case_title"] ?? row["col_3"] ?? "",
  ...
  protected getCaseDetailUrl(caseNumber: string): string | null {
    ...
    return `${BREVARD_BASE_URL}/beca/CaseDetail.cfm?caseNum=${encodeURIComponent(caseNumber)}`;
  }
  ...
  private getCaseTypeCode(caseType: ClerkCaseType): string {
    switch (caseType) {
      case "civil": return "CA";
      case "probate": return "CP";
      case "family": return "DR";
      case "foreclosure": return "CF";
      default: return "CA";
    }
  }
  ...
  private extractParties(row: Record<string, string>): string[] {
    ...
    if (parties.length === 0) {
      const style = row["style"] ?? row["case_title"] ?? "";
